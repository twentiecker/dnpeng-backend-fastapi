...
from app.features.files.repository import (
    get_all_files,
    get_files_category,
    save,
)
...
def get_recent_files(limit: int, db: Session):
    ...
    sorted_files = sorted(
        enriched_files, key=lambda x: (x["date_obj"], x["filename"]), reverse=True
    )
...
def get_files_by_category(category: str, db: Session):
    files = get_files_category(category, db)
