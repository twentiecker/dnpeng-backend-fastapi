...
from app.features.pkp import repository as repo
...
def add_pkp(
    ...
    return repo.create_pkp(db, data)


def get_pkp_data(db: Session, kode: str | None, periode: str | None):
    tahun = None
    freq = None
    period = None
    if periode:
        tahun, freq, period = parse_periode(periode)
    return repo.filter_pkp(db, kode, tahun, freq, period)


def get_pkp_kode(db: Session, kode: str):
    return repo.get_pkp_by_kode(db, kode)


def get_pkp_periode(db: Session, periode: str):
    return repo.get_pkp_by_periode(db, periode)
