...
from app.features.eksim import repository as repo
...
def add_eksim(
    ...
    return repo.create_eksim(db, data)


def get_eksim_data(db: Session, kode: str | None, periode: str | None):
    tahun = None
    freq = None
    period = None
    if periode:
        tahun, freq, period = parse_periode(periode)
    return repo.filter_eksim(db, kode, tahun, freq, period)


def get_eksim_kode(db: Session, kode: str):
    return repo.get_eksim_by_kode(db, kode)


def get_eksim_periode(db: Session, periode: str):
    return repo.get_eksim_by_periode(db, periode)
